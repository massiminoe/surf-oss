...
//!
//! Configure via env `OSX_SURF_GAME_DIR` (folder containing `cstrike/` and/or `hl2/`),
//! or `OSX_SURF_MATERIALS` (parent of a `materials/` tree). See `docs/M3-HANDOFF.md`.
...
impl StockFs {
    pub fn from_env() -> Self {
        ...
        if let Ok(dir) = std::env::var("OSX_SURF_GAME_DIR") {
        ...
        if let Ok(dir) = std::env::var("OSX_SURF_MATERIALS") {
            let p = PathBuf::from(dir);
            if p.is_dir() {
                roots.push(p);
            }
